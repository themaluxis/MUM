...
def test_new_server_connection():
    ...
    # Basic validation
    required_fields = ['service_type', 'url']
    ...
    try:
        ...
        if not service:
            return jsonify({'success': False, 'message': 'Unsupported service type'})
        
        success, message = service.test_connection()
        return jsonify({'success': success, 'message': message})

    except Exception as e:
        current_app.logger.error(f"Error testing new server connection: {e}")
        return jsonify({'success': False, 'message': str(e)})

@bp.route('/api/servers/<int:server_id>/test')
@login_required
@setup_required
@permission_required('manage_media_servers')
...
@bp.route('/api/servers/<int:server_id>/sync/libraries')
@login_required
@setup_required
@permission_required('manage_media_servers')
def sync_server_libraries(server_id):
    """Sync libraries for a media server"""
    result = MediaServiceManager.sync_server_libraries(server_id)
    return jsonify(result)

@bp.route('/api/servers/<int:server_id>/sync/users')
@login_required
@setup_required
@permission_required('manage_media_servers')
def sync_server_users(server_id):
    """Sync users for a media server"""
    result = MediaServiceManager.sync_server_users(server_id)
    return jsonify(result)

@bp.route('/api/plugins/refresh-servers-count', methods=['POST'])
def refresh_plugin_servers_count():
    """Refresh the servers_count for all plugins - useful for fixing sync issues"""
    # Check authentication - allow during setup, require login after setup is complete
    if not current_user.is_authenticated:
        from app.models import AdminAccount
        try:
            admin_exists = AdminAccount.query.first() is not None
    ...
    try:
        ...
        # Update servers_count for all plugins
            ...
            try:
                ...
                if service_type:
                    ...
                    if plugin.servers_count != actual_count:
                        ...
                        db.session.add(plugin)
                        updated_count += 1
                        current_app.logger.info(f"Updated {plugin.plugin_id} servers_count from {plugin.servers_count} to {actual_count}")
                else:
        ...
        return jsonify({
            'success': True, 
            'message': f'Refreshed servers_count for {updated_count} plugins',
